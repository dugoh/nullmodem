...
static void nullmodem_timer_proc(struct timer_list *tl)
{
        int i;
        unsigned long flags;
...
static ssize_t nullmodem_write(struct tty_struct *tty, const unsigned char *buffer, size_t count)
{
        struct nullmodem_end *end = tty->driver_data;
        ssize_t written = 0;
        ...
        //dprintf("%s - #%d %zu bytes --> %zd written\n", __FUNCTION__, tty->index, count, written);
        return written;
}

static unsigned int nullmodem_write_room(struct tty_struct *tty)
{
        struct nullmodem_end *end = tty->driver_data;
        unsigned int room = 0;
...
static void nullmodem_set_termios(struct tty_struct *tty, const struct ktermios *old_termios)
{
        struct nullmodem_end *end = tty->driver_data;
        unsigned long flags;
        unsigned int cflag;
...
static int __init nullmodem_init(void)
{
        ...
        if (retval)
        {
                ...
                tty_driver_kref_put(nullmodem_tty_driver);
                return retval;
        }
